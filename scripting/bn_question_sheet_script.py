...
da_wb = load_workbook("../outputs/sampling/sampled_translations_checked.xlsx")
da_ws = da_wb.active
...
finish_qsts = open_txt("../outputs/translations/fi/dev_questions_fi.txt")
...
for row in da_ws.iter_rows(min_row=2, max_row=da_ws.max_row, min_col=1, max_col=2, values_only=True):
    ...
    bn_qst = finish_qsts[index]
    questions_for_sampling.append([eng_qst,bn_qst])


bn_wb = Workbook()
bn_ws = bn_wb.active
bn_ws.append(["English", "Finnish"])
for qsts in questions_for_sampling:
    bn_ws.append(qsts)

bn_wb.save("../outputs/sampling/sampled_translations_fi.xlsx")
